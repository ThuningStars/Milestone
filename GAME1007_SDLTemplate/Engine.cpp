...
int Engine::Init(const char* title, int xPos, int yPos, int width, int height, int flags)
{
	...
	srand(time(NULL));
	m_enemysp = { {0,m_srcHeight*3,m_srcWidth,m_srcHeight}, {1024,(rand() % 640),m_dstWidth / 5,m_dstHeight / 5} };
...
void Engine::Update()
{
	...
	if (m_timer >= 60)
	{
		m_enemy.push_back(new Enemy ({ 1200, rand() % 700 }));
		m_enemy.shrink_to_fit();
		cout << " New Enemy " << m_enemy.capacity() << endl;
		m_timer = 0;
	}
	...
	for (unsigned i = 0; i < m_enemy.size(); i++) // size() is actual filled numbers of elements
		m_enemy[i]->UpdateEnemyB();
	for (unsigned i = 0; i < m_enemy.size(); i++) // size() is actual filled numbers of elements
	{
		if (m_enemy[i]->GetRekt()->x < -100)
		{
			delete m_enemy[i]; // flag for reallocation
			m_enemy[i] = nullptr; // get rid of the dangling pointer
	...
	if (SDL_HasIntersection(&m_player.m_dst, &m_enemysp.m_pFireDst))
...
void Engine::Render()
{
	...
	for (unsigned i = 0; i < m_enemy.size(); i++) // size() is actual filled numbers of elements
		m_enemy[i]->Render(m_pRenderer, m_pEnemyTexture, &m_enemysp.m_src);
